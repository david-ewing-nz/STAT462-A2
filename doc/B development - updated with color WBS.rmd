...
```{r wsb-step-01}
### Work Breakdown Structure: Question 2 – Discriminant Analysis with Colour Data

1. Load the colour dataset
```{r step-01-load-color, message=FALSE, warning=FALSE}
# TODO: Load color data (e.g., colors_train.csv)
# c_data <- read.csv(...)
```

2. Filter relevant columns and remove missing values
```{r step-02-clean-data}
# TODO: Select features and remove NAs
# c_data <- c_data %>% select(...) %>% drop_na()
```

3. Create outcome variable `c_class` and define predictors
```{r step-03-define-vars}
# TODO: Define outcome and predictor variables
# c_data$c_class <- as.factor(...)
# predictors <- c('feature1', 'feature2', ...)
```

4. Split dataset into training and test sets
```{r step-04-split-data}
# TODO: Split data (70% training, 30% test)
# set.seed(462)
# split <- initial_split(c_data, prop = 0.7, strata = c_class)
# cp_train <- training(split)
# cp_test <- testing(split)
```

5. Fit Quadratic Discriminant Analysis (QDA) model
```{r step-05-fit-qda}
# TODO: Fit QDA model using training data
# cp_qda_model <- discrim_regularized() %>%
#   set_engine("MASS") %>%
#   fit(c_class ~ ., data = cp_train)
```

6. Predict on test set and evaluate
```{r step-06-predict-eval}
# TODO: Make predictions and evaluate performance
# cp_pred_class <- predict(cp_qda_model, new_data = cp_test)
# cp_conf_mat <- conf_mat(cp_test, truth = c_class, estimate = .pred_class)
# cp_conf_mat
```

7. Generate ROC curve and compute AUC
```{r step-07-roc-auc}
# TODO: Get predicted probabilities and plot ROC
# cp_pred_probs <- predict(cp_qda_model, new_data = cp_test, type = "prob")
# roc_auc(cp_test, truth = c_class, .pred_class_1)
# autoplot(roc_curve(...))
```

8. Visualize class separation
```{r step-08-visualize}
# TODO: PCA or feature plots colored by class
# ggplot(cp_train, aes(x = ..., y = ..., color = c_class)) + geom_point()
```

9. Interpret and discuss findings
```{r step-09-interpret}
# TODO: Add textual commentary here or in markdown
# e.g., Which features discriminate best?
```
